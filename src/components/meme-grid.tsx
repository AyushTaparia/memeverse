...
export function MemeGrid({ memes, loading = false, onLoadMore, hasMore = false }: MemeGridProps) {
        ...
        {loading && hasMore && (
          <>
            {Array.from({ length: 4 }).map((_, index) => (
              <div key={`skeleton-${index}`} className="rounded-lg overflow-hidden border bg-card shadow-sm">
                <div className="p-2">
                  <div className="flex items-center gap-2 mb-2">
                    <Skeleton className="w-8 h-8 rounded-full" />
                    <div className="space-y-1">
